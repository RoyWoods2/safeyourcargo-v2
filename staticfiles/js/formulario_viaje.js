...
// === Helpers de opciones con preservación de selección ===
function setSelectOptionsPreserve(selectEl, options, placeholder='Seleccione una opción') {
  if (!selectEl) return;
  ...
  if (values.includes(prev)) {
    selectEl.value = prev;           // conserva la selección previa si sigue siendo válida
  } else if (selectEl.options.length > 1) {
    selectEl.selectedIndex = 1;      // primera opción real
  }
  selectEl.dispatchEvent(new Event('change'));
}

// === Opciones compartidas para Marítimo y Terrestre/Ferroviario ===
const SHARED_EMBALAJE = [
  { value: 'FCL', label: 'FCL' },
  { value: 'LCL', label: 'LCL' },
];

// Aplica opciones compartidas solo si el select no coincide ya
function ensureSharedEmbalaje(selectEl) {
  if (!selectEl) return;
  const currentValues = Array.from(selectEl.options).map(o => o.value).filter(Boolean);
  const needed = SHARED_EMBALAJE.map(o => o.value);
  const sameSet = currentValues.length === needed.length && needed.every(v => currentValues.includes(v));
  if (!sameSet) setSelectOptionsPreserve(selectEl, SHARED_EMBALAJE);
}
...
function actualizarVistaEmbalaje() {
  ...
  if (valorModo === "Aereo") {
    ...
    // ⬇️ NUEVO: Marítimo comparte opciones con Terrestre/Ferroviario
    ensureSharedEmbalaje(embalajeMaritimo);
    ...
    // ⬇️ NUEVO: Terrestre/Ferrov. comparte opciones con Marítimo
    ensureSharedEmbalaje(tipoEmbalajeTerrestre);
